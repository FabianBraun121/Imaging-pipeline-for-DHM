...
class Pipeline:
        ...
        for po in self.positions:
            ...
            for t in self.timesteps:
                ...
                if not self.image_settings_updated:
                    cfg.set_image_variables((cfg.KOALA_HOST.GetPhaseWidth(),cfg.KOALA_HOST.GetPhaseHeight()), cfg.KOALA_HOST.GetPxSizeUm(), cfg.KOALA_HOST.GetLambdaNm(0))
    ...
    def _save_image(self, phase_image: Image, po: Position, save_pos_folder: Path, t: int):
        ...
        if cfg.save_format == ".tif":
            dhm_ph_fname = save_pos_folder + os.sep + str(t).zfill(5) + "_PH" + cfg.save_format
            ...
            if cfg.ph_image:
                ph_fname = save_pos_folder + os.sep + str(t).zfill(5) + "_PC" + cfg.save_format
