# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 17:54:36 2023
...
class Hologram:
    def __init__(self,
        ...
        self.out_of_focus = out_of_focus
...
class SpatialPhaseAveraging:
    def __init__(self,
                 ...
                 timestep: int,
                 num_out_of_focus: int,
                 ):
        
        self.position: Position = position
        self.timestep: int = timestep
        self.placements: List[Placement] = placements
        self.num_out_of_focus = num_out_of_focus
        self.list_out_of_focus = list(np.random.choice(range(len(placements)), num_out_of_focus, replace=False))
    ...
    def _generate_holograms(self) -> List[Hologram]:
        ...
        for i, pl in enumerate(self.placements):
            ...
            out_of_focus = False
            if i in self.list_out_of_focus:
                out_of_focus = True
            hologram = Hologram(fname, pl, out_of_focus=out_of_focus)
...
class Pipeline:
    def __init__(
            ...
            out_of_focus_list: list = list(np.arange(11)),
            ):
        
        self.out_of_focus_list = list(out_of_focus_list)
    ...
    def process(self):
        ...
        for po in self.positions:
            ...
            for t in self.timesteps:
                for o in self.out_of_focus_list:
                    start_image = time.time()
                    spa = SpatialPhaseAveraging(po, placements, t, num_out_of_focus=o)
                    ...
                    self._save_image(phase_image, save_pos_folder, t, o)
    ...
    def _save_image(self, phase_image, save_pos_folder, t, o):
        ...
        if cfg.save_format == ".bin":
            save_folder = save_pos_folder + os.sep + f'num_out_of_focus{str(o).zfill(2)}'
            if not os.path.exists(save_folder):
                os.mkdir(save_folder)
            fname = save_folder + os.sep + "ph_timestep_" + str(t).zfill(5) + cfg.save_format
