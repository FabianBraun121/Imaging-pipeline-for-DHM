...
ax3.plot(foci, unity_based_normalization(std_amp), 'g', label='amplitude std.')
ax3.plot(np.ones(2)*foci[np.argmin(std_amp)], np.array([0,1]), 'g--')
ax3.plot(foci, unity_based_normalization(ph_sharpness), 'b', label='phase sharpness')
ax3.plot(np.ones(2)*foci[np.argmax(ph_sharpness)], np.array([0,1]), 'b--')
ax3.set_xlabel('reconstruction distance [cm]', fontsize=12)
