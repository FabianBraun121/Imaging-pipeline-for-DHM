...
ax1.set_ylabel('Weighted binary crossentropy loss []', fontsize=16)
...
ax1.tick_params(axis='x', labelsize=14)  # Change x-axis tick label size
ax1.tick_params(axis='y', labelsize=14)  

# Create a secondary y-axis for accuracy
ax2 = ax1.twinx()
ax2.plot(np.array(history['unstack_acc'])*100, label='Training accuracy', color=light_blue)
ax2.plot(np.array(history['val_unstack_acc'])*100, label='Validation accuracy', color=light_green)
ax2.set_ylabel('Pixel accuracy [%]', fontsize=16)
# ax2.tick_params(axis='y', labelcolor='green')

# Set y-axis boundaries for accuracy
ax2.set_ylim(bottom=99, top=100)  # Adjust the values as needed
