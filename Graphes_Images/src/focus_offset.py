# -*- coding: utf-8 -*-
"""
Created on Mon Jun 26 08:49:40 2023
...
class Hologram:
    ...
    def set_focus_offset(self, offset):
        self.focus = self.focus + offset
...
save_path = base_path + r'\Graphes_Images\focus_offset'
...
images_in_focus = np.array([np.angle(cplx_img) for cplx_img in spa.cplx_images_list])
avg_images_in_focus = spa.get_phase_avg()
np.save(save_path + os.sep + 'images_in_focus', images_in_focus)
np.save(save_path + os.sep + 'avg_images_in_focus', avg_images_in_focus)

np.random.seed(1)
offsets = np.random.normal(0,0.5,25) # mu, sigma, size
for i, offset in enumerate(offsets):
    spa.holo_list_in_use[i].set_focus_offset(offset)
spa.recalculate_cplx_avg()
images_out_of_focus = np.array([np.angle(cplx_img) for cplx_img in spa.cplx_images_list])
avg_images_out_of_focus = spa.get_phase_avg()
np.save(save_path + os.sep + 'offsets', offsets)
np.save(save_path + os.sep + 'images_out_of_focus', images_out_of_focus)
np.save(save_path + os.sep + 'avg_images_out_of_focus', avg_images_out_of_focus)
#%%
images_in_focus = np.load(save_path + os.sep + 'images_in_focus.npy')
avg_images_in_focus = np.load(save_path + os.sep + 'avg_images_in_focus.npy')
offsets = np.load(save_path + os.sep + 'offsets.npy')
images_out_of_focus = np.load(save_path + os.sep + 'images_out_of_focus.npy')
avg_images_out_of_focus = np.load(save_path + os.sep + 'avg_images_out_of_focus.npy')
#%%
out_of_focus_images = 5
image_mix = [images_out_of_focus[i] for i in range(out_of_focus_images)]
image_mix += [images_in_focus[i] for i in range(out_of_focus_images, 25)]
avg = np.mean(np.array(image_mix), axis=0)
plt.imshow(avg_images_in_focus-avg)
